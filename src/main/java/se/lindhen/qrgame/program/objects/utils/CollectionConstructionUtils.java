...
public class CollectionConstructionUtils {
    ...
    public static Type innerTypeFromArguments(List<Expression> arguments) {
    ...
    public static ValidationResult validateCollection(List<Expression> arguments, ParserRuleContext ctx) {
        if (arguments.size() == 0) {
        ...
        if (arguments.get(0).getType().isType()) {
            if (arguments.size() == 1) {
                ...
                if (arguments.get(1).getType().isNumber()) {
                    return ValidationResult.valid();
                } else {
                    return ValidationResult.invalid(ctx, "Second parameter to a typed collection constructor can only be an integer denoting the size. Got '" + arguments.get(1).getType());
                }
            } else {
                return ValidationResult.invalid(ctx, "Typed collection constructor can only accept 1 or 2 arguments. Got '" + arguments.size());
            }
        }
        Type firstArgType = arguments.get(0).getType();
