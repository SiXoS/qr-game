...
public class TreeMapClass extends QgClass<TreeMapClass.TreeMapValue> {
    ...
    protected List<Method<TreeMapValue>> getMethods() {
        ...
        methods.add(new LambdaMethod<>( "get", (obj, args, prog) -> obj.get(args.get(0).calculate(prog)), new FunctionType(new GenericType(1), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "getOrDefault", (obj, args, prog) -> obj.getOrDefault(args.get(0).calculate(prog), args.get(1).calculate(prog)), new FunctionType(new GenericType(1), objectType, new GenericType(0),new GenericType(1))));
        methods.add(new LambdaMethod<>( "put", (obj, args, prog) -> obj.put(args.get(0).calculate(prog), args.get(1).calculate(prog)), new FunctionType(new GenericType(1), objectType, new GenericType(0),new GenericType(1))));
        methods.add(new LambdaMethod<>( "containsKey", (obj, args, prog) -> obj.containsKey(args.get(0).calculate(prog)), new FunctionType(BoolType.BOOL_TYPE, objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "removeKey", (obj, args, prog) -> obj.removeKey(args.get(0).calculate(prog)), new FunctionType(new GenericType(1), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "remove", (obj, args, prog) -> obj.remove(args.get(0).calculate(prog), args.get(1).calculate(prog)), new FunctionType(BoolType.BOOL_TYPE, objectType, new GenericType(0),new GenericType(1))));
        methods.add(new LambdaMethod<>( "nextKey", (obj, args, prog) -> obj.nextKey(args.get(0).calculate(prog)), new FunctionType(new GenericType(0), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "previousKey", (obj, args, prog) -> obj.previousKey(args.get(0).calculate(prog)), new FunctionType(new GenericType(0), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "nextEntry", (obj, args, prog) -> obj.nextEntry(args.get(0).calculate(prog)), new FunctionType(entryType(), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "previousEntry", (obj, args, prog) -> obj.previousEntry(args.get(0).calculate(prog)), new FunctionType(entryType(), objectType, new GenericType(0))));
        methods.add(new LambdaMethod<>( "firstKey", (obj, args, prog) -> obj.firstKey(), new FunctionType(new GenericType(0), objectType)));
        methods.add(new LambdaMethod<>( "lastKey", (obj, args, prog) -> obj.lastKey(), new FunctionType(new GenericType(0), objectType)));
        methods.add(new LambdaMethod<>( "firstEntry", (obj, args, prog) -> obj.firstEntry(), new FunctionType(entryType(), objectType)));
        methods.add(new LambdaMethod<>( "lastEntry", (obj, args, prog) -> obj.lastEntry(), new FunctionType(entryType(), objectType)));
        methods.add(new LambdaMethod<>( "values", (obj, args, prog) -> obj.values(), new FunctionType(TypeUtils.listWithGenericType(1), objectType)));
        methods.add(new LambdaMethod<>( "keys", (obj, args, prog) -> obj.keys(), new FunctionType(TypeUtils.setWithGenericType(TreeSetClass.getQgClass(), 0), objectType)));
        ...
        methods.add(new ForEachMethod<>("removeAllKeys", TreeMapValue::removeKey, objectType, 0));
        return methods;
    }
